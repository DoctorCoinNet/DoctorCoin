...
#else
    const char* pszModule = "doctorcoin";
#endif
    if (pex)
        return strprintf(
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "doctorcoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "doctorcoind.pid"));
